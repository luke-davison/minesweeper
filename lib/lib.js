...
function drawBoard (boardNode) {
  if (boardSize === "small") {
    boardNode.style.width = boardWidth * 60 + 'px'
  }
  else if (boardSize === "medium") {
    boardNode.style.width = boardWidth * 35 + 'px'
  }
  else {
    boardNode.style.width = boardWidth * 29 + 'px'
  }
...
function showCell (evt) {
  ...
  evt.target.classList.remove("emptyhint")
...
function markCell (evt) {
  ...
  evt.target.classList.remove("minehint")
  var idx = getCellIndex(getRow(evt.target), getCol(evt.target))
  var cell = board.cells[idx]
  ...
  document.getElementById("minesLeft").innerHTML= "<p>" +mineCount + " mines remaining </p>";
  checkForWin();
}
